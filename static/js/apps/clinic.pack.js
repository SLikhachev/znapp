...
const talonOpt= {
  ...
  getOptions: async function() {
    ...
    await moModel$1.getData( talonOpt );
  }
};
...
const moTalon = {
  ...
  getTalon(model, card, talon) {
    ...
    if ( !isNaN(tal) && tal !== 0) {
      ...
      return moModel$1.getViewRpcMap(
        model, [ t1, t2 ]
...
const cardFind= function (vnode) {
  ...
  const findCards= function(event) {  
    ...
    event.preventDefault();
    let data = moModel$1.getFormData( $('form#card_find') );
    //console.log ( data );
    //return false;
    data._tbl= moCardsList.crdTable();
    data.lim = 50;
    data.offs = 1;
    moModel$1.getViewRpc( model, data );
    return false;
  };
  return {
    view () {
...
const vuCardsList = function (vnode) {
  ...
  moModel$1.getViewRpc(model, {_tbl: table_id}, restClinic.cards_cnt.url, restClinic.cards_cnt.method );
  const sort= '';
...
const talonFind = function(vnode){
  ...
  const findTalons= function(event) {  
    ...
    let data = moModel$1.getFormData( event.target );
    ...
    */
    ...
    delete data.q_year;
    //console.log ( data );
    return moModel$1.getViewRpc( model, data );
    //return false;
  };
  const changeYear= e=> {
...
const vuTalonsList = function (vnode) {
  ...
  //console.log(yy);
  moModel$1.getViewRpc(model, { _tbl: yy }, restClinic.talons_cnt.url, restClinic.talons_cnt.method );
...
const pmuForm = function (vnode) {
  ...
  const on_submit = event=> {
    ...
    if ( q == 'grup' ) {
      ...
      _pmu.method= 'POST';
      return moModel$1.getViewRpc(_pmu, { grup: _pmu[q] } ).then(t=> {
        ...
        return moModel$1.getViewRpc(md, items);
      }).then(t=> {
    ...
    return moModel$1.getList( _pmu ).then( t=>{
      ...
      md.headers= {Prefer: 'return=representation'};
      // promise
      return moModel$1.formSubmit(event, md, 'POST');
...
const talPmu = function(vnode) {
  ...
  const add_kol_usl= e=> {
    let { p, url } = kol_usl(e);
    let md= {};
    return moModel$1.getViewRpc( md, { kol_usl: p.kol_usl }, url, 'PATCH' ).then( t=> {
       p.kol_usl += 1;
       return true;
    });
    //return false;    
  ...
  const del_kol_usl= e=> {
    ...
    let md= {};
    if (p.kol_usl == 1) {
      return moModel$1.getViewRpc( md, {}, url, 'DELETE' ).then( t => {
...
const toSaveTalon= async function (tal, check) {
  ...
  if (cons || hosp) {
    ...
    try {
      let r= '';
      let t= await moModel$1.getData(_mdl);
      //console.log(_mdl);
...
const talForm = function (vnode) {
  ...
  const set_ds= (ds, _model)=> e=> {
    ...
    if ( diag.test(tal[ds]) ) {
      ...
      //console.log(ds);
      return moModel$1.getList(_model);// .then(t=> console.log( ds_model.list ));
    }
    //return true;
    return false;
  };
